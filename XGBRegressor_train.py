...
# Dataset generation
# -------------------------
def load_dataset(n_samples=25000):
    import time
    import pickle
    X, y = [], []
    try:
...
def train_xgb():
    ...
    model = xgb.XGBRegressor(
        n_estimators=20000,
        max_depth=5, # 5 originally
        learning_rate=0.01,
        subsample=0.8, # 0.8 originally
        colsample_bytree=0.8, # 0.8 originally
...
if __name__ == "__main__":
    ...
    # Predict
    answer = exact_win_probability(hero_cards=hole, board_cards=flop)
    exact_equity = answer['Win probability'] + answer['Tie probability']
    print("exact_equity: "+str(exact_equity))
    equity = xgb_loaded.predict([features])[0]
    print("Predicted equity:", equity)
