...
def reset_client_window(debug = False):
    ...
    if clint_pos != [-1, -1]:
        while True:
            ...
            pyautogui.mouseDown()
            time.sleep(.174) 
            pyautogui.dragTo(x=def_clint[0]+20, y=def_clint[1]+20, duration=.33, button='left')
            time.sleep(.174) 
            pyautogui.dragTo(x=pull_to[0], y=pull_to[1], duration=.33, button='left')
            pyautogui.mouseUp()
            time.sleep(.2) 
            clint_pos = imagesearch('images/GGPoker_new.png', precision=0.85, debug=debug, calling_function= 'reset_client_window')
            if clint_pos == def_clint:
...
def push_holdem():
    ...
    for _ in range(3):
        ...
        if not l_info_read:
            see_if_there_is_l_info()
        push_holdem_pos = imagesearch('images/holdem.png', precision=.85, debug = False, debug_2 = False, calling_function="push_holdem")
    ...
    def click_selection_or_exit(big_blind="200", first_start_up = False): # development game
        if big_blind == "200": #todo : use tesseract to pick game from  big-blind and buy-in
            ...
            if not click_two_times_please(image_path, precision=.8, debug = False):
                ...
                if not click_two_times_please("images/5k_new.png", precision=8, debug = False):
            ...
            try:
                if first_start_up:
                    image_path = 'images/10k.png' # development game
                    first_start_up = False
                else:
