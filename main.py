...
class AppDelegate(NSObject):
    ...
    def updatePDbyNumber(self): # needs testing
        print("\nupdatePDbyNumber self.readAllPD : "+str(self.readAllPD))
        with self.valset_lock:
            self.readAllPD -= 1
        with self.player_data_lock:
    ...
    def runitup_(self, sender):
        ...
        self.timer2 = NSTimer.alloc().initWithFireDate_interval_target_selector_userInfo_repeats_(start_time, 3.75, self, 'gameScreenshot:', None, True)
            ...
            if bet_equity - call_equity > 0.0765:
                ...
                if call_equity > 0.215:
                    ...
                    if bet_equity > 0.25:
                        ...
                        if call_equity > -0.15 or bet_equity > -0.15: # fixing folding too much issue, but actually this is sus (too low value)
                            print("call equity decent enough to call")
                            return "call"
                        else:
                            if not prod:
                                if random.randrange(2) == 0 and call_equity > -0.25: # fixing folding too much issue, but actually this is sus (too low value)
                                    print("rando-call")
                                    return "call"
                                elif random.randrange(2) == 0 and bet_equity > -0.25:
                                    print("rando-raise")
                                    return "raise1"
