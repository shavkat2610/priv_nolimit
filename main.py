...
class AppDelegate(NSObject):
    ...
    def runitup_(self, sender):
        ...
        self.timer2 = NSTimer.alloc().initWithFireDate_interval_target_selector_userInfo_repeats_(start_time, 1.5, self, 'gameScreenshot:', None, True)
            ...
            if self_dec == "fold":
                if to_call < 0.1:
                    ...
                    if self.potheight > 2.0:
                        ...
                        with self.mod_writing_lock:
                            self.made_model_output = True
                            self.model_output = -0.5 * difference # -0.15
                    with self.cards_lock:
                        self.cards_open = False
                    with self.to_call_lock:
                        self.to_call = 0.0
